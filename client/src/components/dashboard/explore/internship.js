...
class Internship extends Component {
    componentWillMount(){
      this.props.fetchAllInternshipChances();
    }
    renderList() {
      if(this.props.events){
        if(this.props.events.length > 0) {
          return this.props.events.map(event => {
            ...
            return (
                <Link to={ {pathname: `/dashboard/explore/internship/${event._id}`, query:eventQuery }}  key={event._id}>
...
function mapStateToProps({ internship }){
  return {
    events: internship.events
  }
}
export default connect(mapStateToProps, actions)(Internship);