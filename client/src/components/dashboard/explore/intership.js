...
class Intership extends Component {
    componentWillMount(){
      this.props.fetchAllIntershipChances();
    }
    renderList() {
      if(this.props.events){
        if(this.props.events.length > 0) {
          return this.props.events.map(event => {
            ...
            return (
                <Link to={ {pathname: `/dashboard/explore/intership/${event._id}`, query:eventQuery }}  key={event._id}>
                <div className="card resource">
                  ...
                  <div className="body"><div className="card-title">{event.position}</div></div>
                </div>
                </Link>
            )
          })
    ...
    render() {
        return (
            <div className="section">
              ...
              <div className="title">Intership Program</div>
...
function mapStateToProps({ intership }){
  return {
    events: intership.events
  }
}
export default connect(mapStateToProps, actions)(Intership);